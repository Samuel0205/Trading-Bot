...
# EXACT SIGNAL STRUCTURE YOUR UI EXPECTS
def get_signals(ticker, price):
    hist = price_history[ticker]

    if len(hist) < 5:
        return [
            {"name":"MA Crossover","action":"hold","signal":50},
            {"name":"RSI","action":"hold","signal":50},
            {"name":"Bollinger","action":"hold","signal":50},
            {"name":"Mean Reversion","action":"hold","signal":50},
        ]
    ...
    return [
        {
            "name": "MA Crossover",
            "action": "buy" if ma50 > ma200 else "sell" if ma200 > ma50 else "hold",
            "signal": 70
        },
        {
            "name": "RSI",
