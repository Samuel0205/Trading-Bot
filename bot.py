...
def check_stops(ticker, price):
    ...
    # Track high-water mark for all positions (used by trailing stop for non-gap plays too)
    ...
    if not pos.get("partial_done") and gain_pct >= PARTIAL_EXIT_PCT:
        ...
        # Fall through — stop/target checks still run on this tick after partial exit
    ...
    if pos.get("is_gap_play"):
        ...
        # Uses high-water mark so a pullback doesn't lower the trail.
        if gain_pct > 0.05:
            highest    = pos.get("highest_price", price)
            trail_stop = round(highest * (1 - TRAILING_STOP_GAP_PCT), 3)
...
def health_data():
    ...
    return jsonify({
        ...
        "trades_remaining": max(0, MAX_DAILY_TRADES - daily_trade_count),
...
def restore_trade_state():
    """On startup: restore today's daily trade count from filled orders."""
    global daily_trade_count, daily_trade_date
    try:
        today       = now_et().date()
